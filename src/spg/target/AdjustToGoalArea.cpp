#include "spg/target/AdjustToGoalArea.hpp"
#include <algorithm>
#include <cmath>
...
void adjustToGoalArea(SPGState& state) {
    double xpos = 0.5 * state.par.field_goal_area[0];
    double ypos = state.par.field_size[1] * 0.5 - state.par.field_goal_area[1];
    bool is_in_goal_area = std::abs(state.target.p[0]) < xpos && std::abs(state.target.p[1]) > ypos;
    if (is_in_goal_area) {
